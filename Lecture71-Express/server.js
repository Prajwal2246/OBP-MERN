...
app.get("/even_ids", (req, res) => {
  const evenIdProducts = products
    .filter((item) => item.id % 2 === 0)
    .map((item) => ({ id: item.id, title: item.title }));
  res.send(evenIdProducts);
});
...
app.get("/users/:id", (req, res) => {
  ...
  const user = users.find((user) => user.id == req.params.id); //use when first matching elemnt and filter when all matching element
  res.send(user);
});
...
app.get("/users", (req, res) => {
  ...
  res.send(PuneOBP2Users);
});

//post request
app.post("/", (req, res) => {
  console.log(req.body);
  res.send("succesfully saved data");
});
